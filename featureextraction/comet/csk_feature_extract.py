...
class CSKFeatureExtractor():
    ...
    def set_atomic_inputs(self, input_event, category, data_loader,
                          text_encoder):
        XMB = torch.zeros(1, data_loader.max_event + 1).long().to(cfg.device)
        prefix, suffix = data.atomic_data.do_example(text_encoder, input_event,
                                                     None, True, None)
    ...
    def extract(self, sentence):
        atomic_keys = [
            'xIntent', 'xAttr', 'xNeed', 'xWant', 'xEffect', 'xReact', 'oWant',
            'oEffect', 'oReact'
        ]
        map1 = [{}, {}, {}, {}, {}, {}, {}, {}, {}]
            ...
            for x in sentence[item]:
                ...
                for sent in tokenize.sent_tokenize(input_event):
                    ...
                    XMB = model_utils.prepare_position_embeddings(
                        self.opt, self.data_loader.vocab_encoder,
                        XMB.unsqueeze(-1))
