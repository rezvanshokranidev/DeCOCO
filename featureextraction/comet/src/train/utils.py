...
def update_generation_losses(losses, nums, micro, macro, bs, length, loss):
    ...
    if isinstance(length, int):
        update_indiv_generation_losses(losses, nums, micro, macro, bs, length,
                                       loss)
    else:
        update_tensor_generation_losses(losses, nums, micro, macro, bs, length,
                                        loss)


def update_indiv_generation_losses(losses, nums, micro, macro, bs, length,
                                   loss):
...
def update_tensor_generation_losses(losses, nums, micro, macro, bs, length,
                                    loss):
