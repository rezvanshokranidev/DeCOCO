...
def train_or_eval_model(model,
    ...
    for data in dataloader:
        ...
        log_prob = model(r, x5, x6, x1, o2, o3, qmask, umask, att2=True)
    ...
    if preds != []:
        ...
        return float('nan'), float('nan'), float('nan'), [], [], [], float(
            'nan'), []
...
if __name__ == '__main__':
    ...
    parser.add_argument('--active-listener',
                        action='store_true',
                        default=False,
    ...
    emo_gru = True
    n_classes = 6
    # 'Happiness','Sadness','Neutral','Anger','Excited','Frustration'
    cuda = args.cuda
    ...
    D_m = 1024  # input dimension of roberta features
    D_s = 768  # input dimension of comet features
    D_g = 150  # output dimension of context state GRU(c)
    D_p = 150  # output dimension of internal state GRU(q)
    D_r = 150  # output dimension of external state GRU(r)
    D_i = 150  # output dimension of intent state GRU(i)
    D_h = 100  # intermediate hidden dimension
    D_a = 100  # attention dimension
    D_ca = 100  # attention dimension of context attention

    D_e = D_p + D_r + D_i  # output dimension of emotion state GRU(e)
    ...
    print('IEMOCAP DeCOCo Model.')

    if cuda:
        model.cuda()

    loss_weights = torch.FloatTensor([
        1 / 0.086747, 1 / 0.144406, 1 / 0.227883, 1 / 0.160585, 1 / 0.127711,
        1 / 0.252668
    ])
    ...
    dir = path.cwd()
    lf = open(dir / 'train/logs/decoco_iemocap_logs.txt', 'a')
    ...
    for e in range(n_epochs):
        ...
        train_loss, train_acc, train_fscore = train_or_eval_model(
            model, loss_function, train_loader, e, optimizer, True)
        valid_loss, valid_acc, valid_fscore = train_or_eval_model(
            model, loss_function, valid_loader, e)
        test_loss, test_acc, test_fscore = train_or_eval_model(
        ...
        x = 'epoch: {}, train_loss: {}, acc: {}, fscore: {}, valid_loss: {}, acc: {}, fscore: {}, test_loss: {}, acc: {}, fscore: {}, time: {} sec'.format(
            e + 1, train_loss, train_acc, train_fscore, valid_loss, valid_acc,
            valid_fscore, test_loss, test_acc, test_fscore,
    ...
    rf = open(dir / 'train/results/decoco_iemocap_results.txt', 'a')
